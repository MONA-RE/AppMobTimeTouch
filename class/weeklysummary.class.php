...
?>db->query($sql);
    ...
    /**
     * Calculate week start and end dates from year and week number
     *
     * @return void
     */
    public function calculateWeekDates()
    {
        if (empty($this->year) || empty($this->week_number)) {
            return;
        }

        // Get first day of the year
        $first_day = new DateTime($this->year.'-01-01');
        
        // Find the first Monday of the year
        $first_monday = clone $first_day;
        $first_monday->modify('Monday this week');
        
        // If January 1st is a Monday, we're already on the first Monday
        if ($first_day->format('N') == 1) {
            $first_monday = $first_day;
        }
        ...
        $week_start = clone $first_monday;
        $week_start->modify('+'.($this->week_number - 1).' weeks');
        ...
        $this->week_end_date = $week_end->format('Y-m-d');
    }

    /**
     * Generate weekly summary for a user and week
     *
     * @param int  $user_id     User ID
     * @param int  $year        Year
     * @param int  $week_number Week number
     ...
     * @return int              <0 if KO, >0 if OK (summary ID)
     */
    public function generateWeeklySummary($user_id, $year, $week_number, $user)
    {
        global $conf;

        // Check if summary already exists
        if ($this->summaryExists($user_id, $year, $week_number)) {
            // Fetch existing summary and update it
            $sql = "SELECT rowid FROM ".MAIN_DB_PREFIX.$this->table_element;
            $sql .= " WHERE fk_user = ".((int) $user_id);
            $sql .= " AND year = ".((int) $year);
            $sql .= " AND week_number = ".((int) $week_number);
        ...
        // Get expected hours for this week
        $this->expected_hours = $this->getExpectedHoursForWeek($user_id);

        // Calculate totals from timeclock records
        $this->calculateTotalsFromRecords();
        ...
        if (empty($this->id)) {
            ...
            return $this->update($user) > 0 ? $this->id : -1;
        }
    }
     ...
     * @return void
     */
    private function calculateTotalsFromRecords()
    {
        // Get all completed timeclock records for this week
        $sql = "SELECT COUNT(DISTINCT DATE(clock_in_time)) as days_worked,";
        ...
        $sql .= " AND DATE(clock_in_time) >= '".$this->db->escape($this->week_start_date)."'";
        $sql .= " AND DATE(clock_in_time) <= '".$this->db->escape($this->week_end_date)."'";
        ...
        if ($resql) {
            $obj = $this->db->fetch_object($resql);
            $this->days_worked = $obj->days_worked ? $obj->days_worked : 0;
            $this->total_hours = $obj->total_minutes ? round($obj->total_minutes / 60, 2) : 0;
            $this->total_breaks = $obj->total_break_minutes ? $obj->total_break_minutes : 0;
            $this->db->free($resql);
        } else {
            $this->days_worked = 0;
            $this->total_hours = 0;
    ...
    {
        if ($this->total_hours > $this->expected_hours) {
            $this->overtime_hours = round($this->total_hours - $this->expected_hours, 2);
        } else {
    ...
    /**
     * Get expected hours for a week for a user
     *
     * @param int $user_id User ID
     * @return float       Expected hours
     */
    private function getExpectedHoursForWeek($user_id)
    {
        global $conf;

        // Default to 40 hours per week (5 days * 8 hours)
        $default_weekly_hours = !empty($conf->global->APPMOBTIMETOUCH_WEEKLY_HOURS_THRESHOLD) ? $conf->global->APPMOBTIMETOUCH_WEEKLY_HOURS_THRESHOLD : 40;

        // TODO: In the future, this could be enhanced to:
        // - Check user's contract for specific working hours
        // - Account for holidays and vacations
        // - Handle part-time employees
        // - Consider company-specific working patterns

        return (float) $default_weekly_hours;
    }
    ...
    public function validate($user)
    {
        ...
        if ($this->status == self::STATUS_VALIDATED) {
            $this->error = 'WeeklySummaryAlreadyValidated';
            return -1;
        }
        ...
        if ($resql) {
            ...
            // Call trigger
            if (!$this->call_trigger('WEEKLYSUMMARY_VALIDATE', $user)) {
                $this->db->commit();
                return 1;
            } else {
                $this->db->rollback();
    ...
    /**
     * Set summary to completed status
     *
     * @param User $user User object
     * @return int       <0 if KO, >0 if OK
     */
    public function setCompleted($user)
    {
        if ($this->status == self::STATUS_COMPLETED) {
            return 0;
        }

        return $this->setStatusCommon($user, self::STATUS_COMPLETED, false, 'WEEKLYSUMMARY_COMPLETE');
    }
    ...
    public function cancel($user)
    {
        ...
        return $this->setStatusCommon($user, self::STATUS_CANCELLED, false, 'WEEKLYSUMMARY_CANCEL');
    }

    /**
     * Reopen weekly summary
     *
     * @param User $user User object
     * @return int       <0 if KO, >0 if OK
     */
    public function reopen($user)
    {
        ...
        return $this->setStatusCommon($user, self::STATUS_IN_PROGRESS, false, 'WEEKLYSUMMARY_REOPEN');
    }
     ...
     * @return array             Array of WeeklySummary objects
     */
    public function getWeeklySummariesByUser($user_id, $date_start = '', $date_end = '', $status = '')
    {
        $summaries = array();
    ...
    /**
     * Get weekly summary by user, year, and week
     *
     * @param int $user_id     User ID
     * @param int $year        Year
     * @param int $week_number Week number
     * @return WeeklySummary|null Summary object or null if not found
     */
    public function getWeeklySummaryByUserAndWeek($user_id, $year, $week_number)
    {
        $sql = "SELECT rowid FROM ".MAIN_DB_PREFIX.$this->table_element;
        $sql .= " WHERE fk_user = ".((int) $user_id);
        $sql .= " AND year = ".((int) $year);
